...
CHANCE_CARDS_DECK = [
    {"id": "c1", "action": "move", "target": 5, "text": "Đi vé hạng nhất đến trạm Monopoly Rail.<br>Nếu đi qua ô BẮT ĐẦU, nhận 2,000,000<s>M</s>."},
...
def continue_turn_or_mr_monopoly(session_id, is_double):
    player = game.players.get(session_id)
    if not player or getattr(player, 'is_bankrupt', False):
    ...
    if getattr(player, 'pending_mr_m', False):
        ...
        if target_pos is not None:
            ...
            if getattr(player, 'is_bankrupt', False):
                ...
                emit('show_action_popup', {'die1': '-', 'die2': '-', 'total': 0, 'special': 'Mr.M Move', 'jail_msg': "", 'popup': popup_data, 'is_double': is_double}, room=session_id)
            else:
...
def apply_card_effect(session_id, player, card, prefix_msg, is_double):
    ...
    if action in ["move", "move_nearest_rail", "move_nearest_util", "back_3", "go_jail"]:
        popup_data['type'] = 'CardMove'
        popup_data['card_data'] = card
        return True, popup_data
...
@socketio.on('execute_card_move')
def handle_execute_card_move(data):
    ...
    if not card:
        continue_turn_or_mr_monopoly(session_id, is_double)
        return
    ...
    success, popup_data = process_space(session_id, player, player.position, prefix_msg="", is_double=is_double)
    broadcast_game_state()
    
    if getattr(player, 'is_bankrupt', False):
        end_turn_logic(session_id, False)
    elif success and popup_data:
        emit('show_action_popup', {'die1': '-', 'die2': '-', 'total': 0, 'special': '', 'jail_msg': "", 'popup': popup_data, 'is_double': is_double}, room=session_id)
    else:
        continue_turn_or_mr_monopoly(session_id, is_double)
...
def process_space(session_id, player, space_id, prefix_msg="", is_double=False, dice_total=0):
    ...
    if action_type == "Chance":
        ...
        return apply_card_effect(session_id, player, card, prefix_msg + f"<br>❓ [CƠ HỘI]: {card['text']}<br>", is_double)
    elif action_type == "CommunityChest":
        card = draw_card('chest')
        return apply_card_effect(session_id, player, card, prefix_msg + f"<br>🎁 [KHÍ LỆNH]: {card['text']}<br>", is_double)
    ...
    if action_type in ["Property", "Railroad", "Utility"]:
        if space['owner'] is None:
            ...
            if action_type == "Property":
                ...
                socketio.emit('show_dice_roll', {'die1': u_d1, 'die2': u_d2, 'special_die': u_spec, 'roller': player.name})
                socketio.sleep(1.8)
...
def handle_roll_dice():
    ...
    if getattr(game, 'is_initial_rolling', False):
        ...
        socketio.emit('show_dice_roll', {'die1': die1, 'die2': die2, 'special_die': None, 'roller': player.name})
        socketio.sleep(1.8)
    ...
    socketio.emit('show_dice_roll', {'die1': die1, 'die2': die2, 'special_die': special_die, 'roller': player.name})
    socketio.sleep(1.8)
...
def handle_buy_property(data):
    ...
    if space['owner'] is None and player.balance >= space['price']:
        ...
        broadcast_game_state()
    continue_turn_or_mr_monopoly(session_id, data.get('is_double', False))
...
def resolve_blind_auction():
    ...
    if not valid_bids:
        ...
        continue_turn_or_mr_monopoly(active_auction['initiator_sid'], active_auction['is_double'])
...
def handle_time_out_reached():
    ...
    if game.players and session_id == list(game.players.keys())[0]:
        ...
        broadcast_game_state()
        socketio.emit('show_timeout_decision', room=session_id)
